...
sub probevars {
	...
	return $class->_makevars({
		pings => {
		    ...
		    _sub => sub {
				my $val = shift;
				return "ERROR: for DismanPing, pings must be between 1 and 15"
...
sub ping($) {
	...
	my $ret = &snmpset($host, "pingCtlRowStatus.$idx", "integer", 6); #destroy
	if (!defined($ret)) {
	    Smokeping::do_log("DismanPing: old probe for $t->{vars}{menu} is probably running: " . $SNMP_Session::errmsg);
	    next;
	}

	my $targetaddr = inet_aton($addr);
	if (!defined($targetaddr)) {
	    Smokeping::do_log("DismanPing can't resolve destination address $addr for $t->{vars}{menu}");
	    next;
	}
...
#  differently than pings
	...
	if (defined($t->{vars}{packetsize})) {
	    unshift(@values, "pingCtlDataSize.$idx", "gauge",	$t->{vars}{packetsize});
	}
	if (defined($t->{vars}{pingsrc})) {
	    my $srcaddr = inet_aton($t->{vars}{pingsrc});
	    if (!defined($srcaddr)) {
		Smokeping::do_log("WARNING: DismanPing can't resolve source address $t->{vars}{pingsrc} for $t->{vars}{menu}");
	    } else {
		unshift(@values,
...
#{ use Data::Dumper; open(TMP, ">>/tmp/disman-smoke"); print TMP Dumper(scalar(localtime(time)), $t->{vars}{menu}, \@snmpsetret); close(TMP); }
    ...
    my $setup = time - $start;
    Smokeping::do_debuglog("DismanPing took $setup to set up, now sleeping for $longest");
    sleep($longest);
    my $allok = 0;
    my $startend = time;
    while (!$allok) {
	...
	foreach my $t (@{$self->targets}) {
	    ...
	    if (!defined($status) || $status == 1) {	# if SNMP fails, assume it's not done.
		my $howlong = time - $start;
		if ($howlong > $self->step) {
		    Smokeping::do_log("DismanPing: abandoning $t->{vars}{menu} after $howlong seconds");
		    $pending->{$t->{tree}} = 0;
		} else {
		    Smokeping::do_log("DismanPing: $t->{vars}{menu} is still running after $howlong seconds");
		    $allok = 0;
		}
		next;
	    }
	    ...
	    Smokeping::do_debuglog("DismanPing: table download returned " . (defined($ret) ? $ret : "undef"));
		...
		Smokeping::do_log("DismanPing: $t->{vars}{menu} only returned " . scalar(@times) . " results");
		@times = ();
	    } elsif (@times > $t->{vars}{pings}) {
		Smokeping::do_log("DismanPing: $t->{vars}{menu} returned " . scalar(@times) . " results, taking last $t->{vars}{pings}");
		@times = @times[$#times - $t->{vars}{pings}..$#times];
	    }
	    if (@times) {
		my (@goodtimes) = ();
		foreach $t (@times) {
		    push(@goodtimes, $t->[0]) if ($t->[1] == 1); # responseReceived(1)
		}
